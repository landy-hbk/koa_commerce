...
router.get("/roleList", async (ctx) => {
  await roleSQL(ctx, "query");
});
// 添加菜单数据
router.post("/roleList", validateSchemaJoi("post", roleSchema), async (ctx) => {
  await roleSQL(ctx, "add");
});

// 修改菜单数据
router.put("/roleList", async (ctx) => {
  await roleSQL(ctx, "update");
});

// 删除数据
router.delete("/roleList/:id", async (ctx) => {
  await roleSQL(ctx, "delete");
});

const roleSQL = async (ctx, type) => {
